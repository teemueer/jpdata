import os
import gzip
import xml.etree.ElementTree as ET
import sqlalchemy.dialects.postgresql as postgres
import click
import json
from urllib.request import urlretrieve
from zipfile import ZipFile
from app import db
from app.cli import bp
from app.characters.models import Character, Meaning, Reading, Variant, Decomposition


@bp.cli.group()
def characters():
    """Character commands"""


@characters.command()
def kanjidic():
    """Insert characters from KANJIDIC"""

    url = "http://www.edrdg.org/kanjidic/kanjidic2.xml.gz"

    if not os.path.exists("data"):
        os.makedirs("data")

    filepath = os.path.join("data", "kanjidic2.xml.gz")
    if not os.path.exists(filepath):
        print(f"*** Downloading {url}")
        urlretrieve(url, filepath)

    characters = []
    readings = []
    meanings = []
    variants = []

    def parse_node(node, _type):
        results = {}
        for e in node:
            results[e.get(_type)] = e.text
        return results

    def parse_misc(node, literal):
        misc = {}
        for e in node:
            if e.tag in ("grade", "freq", "jlpt"):
                misc[e.tag] = e.text
            elif e.tag == "stroke_count":
                if "stroke_count" not in misc:
                    misc["stroke_count"] = e.text
            elif e.tag == "rad_name":
                readings.append(
                    {
                        "r_type": "radical",
                        "reading": e.text,
                        "character_literal": literal,
                    }
                )
            elif e.tag == "variant":
                var_type = e.get("var_type")
                if var_type in ("jis208", "jis212", "jis213", "nelson_c"):
                    variants.append(
                        {
                            "var_type": var_type,
                            "variant": e.text,
                            "character_literal": literal,
                        }
                    )
        return misc

    def parse_reading_meaning(node, literal):
        for e in node:
            if e.tag == "rmgroup":
                for rm in e:
                    if rm.tag == "reading":
                        r_type = rm.get("r_type")
                        if r_type in ("ja_on", "ja_kun", "pinyin"):
                            readings.append(
                                {
                                    "r_type": r_type,
                                    "reading": rm.text,
                                    "character_literal": literal,
                                }
                            )
                    elif rm.tag == "meaning":
                        m_lang = rm.get("m_lang", "en")
                        if m_lang == "en":
                            meanings.append(
                                {"meaning": rm.text, "character_literal": literal}
                            )
            elif e.tag == "nanori":
                readings.append(
                    {
                        "r_type": "nanori",
                        "reading": e.text,
                        "character_literal": literal,
                    }
                )

    print(f"*** Parsing KANJIDIC")
    with gzip.open(filepath, "r") as f:
        tree = ET.parse(f)
        for character in tree.iter("character"):
            misc = {}
            dic_refs = {}
            q_codes = {}

            for e in character:
                if e.tag == "literal":
                    literal = e.text
                elif e.tag == "codepoint":
                    codepoints = parse_node(e, "cp_type")
                elif e.tag == "radical":
                    radicals = parse_node(e, "rad_type")
                elif e.tag == "misc":
                    misc = parse_misc(e, literal)
                elif e.tag == "dic_number":
                    dic_refs = parse_node(e, "dr_type")
                elif e.tag == "query_code":
                    q_codes = parse_node(e, "qc_type")
                elif e.tag == "reading_meaning":
                    parse_reading_meaning(e, literal)

            character = {
                "literal": literal,
                "ucs": codepoints["ucs"],
                "jis208": codepoints.get("jis208"),
                "jis212": codepoints.get("jis212"),
                "jis213": codepoints.get("jis213"),
                "radical": chr(0x2F00 + int(radicals.get("classical")) - 1),
                "stroke_count": misc["stroke_count"],
                "grade": misc.get("grade"),
                "freq": misc.get("freq"),
                "jlpt": misc.get("jlpt"),
                "nelson_c": dic_refs.get("nelson_c"),
                "nelson_n": dic_refs.get("nelson_n"),
                "heisig": dic_refs.get("heisig"),
                "heisig6": dic_refs.get("heisig6"),
                "halpern_kkd": dic_refs.get("halpern_kkd"),
                "halpern_kkld_2ed": dic_refs.get("halpern_kkld_2ed"),
            }

            characters.append(character)

    print(f"*** Inserting characters")
    stmt = (
        postgres.insert(Character)
        .values(characters)
        .on_conflict_do_nothing(index_elements=["literal"])
    )
    db.session.execute(stmt)

    print(f"*** Inserting meanings")
    stmt = (
        postgres.insert(Meaning)
        .values(meanings)
        .on_conflict_do_nothing(constraint="uq_meaning_character")
    )
    db.session.execute(stmt)

    print(f"*** Inserting readings")
    stmt = (
        postgres.insert(Reading)
        .values(readings)
        .on_conflict_do_nothing(constraint="uq_type_reading_character")
    )
    db.session.execute(stmt)

    print(f"*** Inserting variants")
    stmt = (
        postgres.insert(Variant)
        .values(variants)
        .on_conflict_do_nothing(constraint="uq_type_variant_character")
    )
    db.session.execute(stmt)

    db.session.commit()


@characters.command()
def heisig():
    """Insert official keywords from Remembering the Kanji"""

    url = (
        "https://raw.githubusercontent.com/sdcr/heisig-kanjis/master/heisig-kanjis.csv"
    )

    if not os.path.exists("data"):
        os.makedirs("data")

    filepath = os.path.join("data", "heisig.csv")
    if not os.path.exists(filepath):
        print(f"*** Downloading {url}")
        urlretrieve(url, filepath)

    characters = []

    print("*** Parsing Heisig keywords")
    with open(filepath, "r", encoding="utf-8") as f:
        for n, line in enumerate(f):
            line = line.strip()
            if not line or line.startswith("#") or n == 0:
                continue

            fields = line.split(",")

            literal = fields[0]
            keyword_5th_ed = fields[3]
            keyword_6th_ed = fields[4]
            heisig_keyword = keyword_6th_ed if keyword_6th_ed else keyword_5th_ed

            characters.append(
                {
                    "literal": literal,
                    "heisig_keyword": heisig_keyword,
                }
            )

    print("*** Inserting keywords")
    db.session.bulk_update_mappings(Character, characters)
    db.session.commit()


@characters.command()
def decomposition():
    """Insert character decomposition data"""

    url = "https://raw.githubusercontent.com/cjkvi/cjkvi-ids/master/ids.txt"

    if not os.path.exists("data"):
        os.makedirs("data")

    filepath = os.path.join("data", "decomp.txt")
    if not os.path.exists(filepath):
        print(f"*** Downloading {url}")
        urlretrieve(url, filepath)

    print("*** Parsing decompositions")
    decompositions = []
    with open(filepath, "r", encoding="utf-8") as f:
        for line in f:
            line = line.strip()

            if not line or line.startswith("#"):
                continue

            fields = line.split("\t")
            literal = fields[1]
            if 0x4E00 > ord(literal) > 0x9FAF:
                continue

            for decomposition in fields[2:]:
                decompositions.append(
                    {
                        "decomposition": decomposition.split("[")[0],
                        "character_literal": literal,
                    }
                )

    print("*** Inserting decompositions")

    valid_literals = {
        character[0] for character in db.session.query(Character.literal).all()
    }

    filtered_decompositions = [
        decomposition
        for decomposition in decompositions
        if decomposition["character_literal"] in valid_literals
    ]

    stmt = (
        postgres.insert(Decomposition)
        .values(filtered_decompositions)
        .on_conflict_do_nothing()
    )

    db.session.execute(stmt)
    db.session.commit()


@characters.command()
def krad():
    """Insert KRAD data"""

    url = "http://ftp.edrdg.org/pub/Nihongo/kradzip.zip"

    if not os.path.exists("data"):
        os.makedirs("data")

    filepath = os.path.join("data", "kradzip.zip")
    if not os.path.exists(filepath):
        print(f"*** Downloading {url}")
        urlretrieve(url, filepath)

    krad = os.path.join("data", "kradfile")
    krad2 = os.path.join("data", "kradfile2")

    if not os.path.exists(krad) and os.path.exists(krad2):
        with ZipFile(filepath) as f:
            f.extractall("data")

    characters = []

    for file in (krad, krad2):
        print(f"*** Parsing {file}")
        with open(file, "r", encoding="euc-jp") as f:
            for line in f:
                line = line.strip()
                if not line or line.startswith("#"):
                    continue

                literal, parts = line.split(" : ")

                characters.append(
                    {
                        "literal": literal,
                        "parts": parts.replace(" ", ""),
                    }
                )

    print("*** Inserting KRAD parts")
    db.session.bulk_update_mappings(Character, characters)
    db.session.commit()


@characters.command()
@click.argument("filepath")
def keywords():
    """Insert my keywords"""

    with open(filepath, "r", encoding="utf-8") as f:
        mnemonics = json.load(f)

    characters = []
    for m in mnemonics:
        characters.append(
            {
                "literal": m["literal"],
                "my_keyword": m["keyword"],
            }
        )

    print("*** Inserting my keywords")
    db.session.bulk_update_mappings(Character, characters)
    db.session.commit()


@characters.command()
@click.argument("filepath")
def kanjikirja(filepath):
    """Insert kanjikirja indexes"""

    with open(filepath, "r", encoding="utf-8") as f:
        kanjikirja = json.load(f)

    print("*** Inserting kanjikirja indexes")
    db.session.bulk_update_mappings(Character, kanjikirja)
    db.session.commit()
